...
- availableTime에 따라 루틴의 전체 시간을 적절하게 배분하세요.
  - '1시간 이내': 전체 30~60분
  - '1~2시간': 전체 60~100분
  - '3시간 이상': 전체 90~150분
- 단, fatigue가 '완전 방전'이거나 mood가 '별로예요'이면 위 범위보다 짧게 구성해도 됩니다.
- availableTime이 많다고 해서 억지로 활동을 채우지 마세요.
- 특히 '3시간 이상'인 경우 3시간을 모두 활동으로 채우지 말고, 회복 활동과 활동 사이에 충분한 휴식이나 자유시간을 둘 수 있도록 구성하세요.
- 시간 여유가 많은 경우 생활 돌보기, 즐거움, 가벼운 외부 활동, 작은 완료, 의도적인 휴식 등을 자연스럽게 조합하세요.
- 하나의 활동을 지나치게 길게 늘리지 말고, 실제 생활에서 자연스럽게 이어지는 여러 단계로 구성하세요.
- 사용자가 실제로 사용할 수 있는 시간을 고려하여 각 step의 minutes를 현실적으로 배분하세요.
- 모든 시간을 활동으로 채우기보다 회복에 필요한 여유 시간도 고려하세요.
...
- title은 반드시 짧은 두 개의 문구를 쉼표(,)로 연결해서 작성하세요.
- 쉼표 앞뒤의 문구는 각각 하나의 자연스러운 제목 구절이 되도록 하세요.
- 예: '여유로운 저녁, 나를 돌보는 회복 루틴'
- 예: '기분 전환을 위한 저녁, 천천히 나를 돌보는 시간'
- 예: '오늘은 조금 천천히, 나를 위한 저녁'
